...
Для запуска нужно перейти в корень проекта и запустить команду **docker compose up** (Можно также запустить контейнеры через Visual Studio Code)<br>
Помимо этого в Makefile есть команды для запуска приложения/тестов<br>
...
В openapi.yml не было такого кода ошибки, поэтому добавил <i>USER_IN_ANOTHER_TEAM</i>. С созданием пулл-реквеста аналогичная ситуация (мы не можем прикрепить пользователя, который находится не в той команде, что автор.
